...
public class PagerSlidingTabStrip extends HorizontalScrollView {
  ...
  private int indicatorColor = Color.parseColor("#ff4551");;
  private int underlineColor = Color.parseColor("#76B2dc");
  private int dividerColor = 0x1AFFFFFF;
...
//        rectPaint);
    canvas.drawLine(lineLeft,height-5, lineRight, height-5, rectPaint);
  ...
  private void setTabColor(int position) {
	  for(int i = 0; i < tabsContainer.getChildCount();i++){
		  ...
		  if(i == position){
			  if(v != null)v.setTextColor(tabTextColor);
		  }else{
			  if(v != null)v.setTextColor(assistTextColor);
		  }
	  }		
	}
