package com.ferris.browser.webview;
...
import com.ferris.browser.webview.WebViewClassic.TitleBarDelegate;
...
public class TitleBarWebView extends WebView implements TitleBarDelegate {
	 ...
	 * {@code setEmbeddedTitleBar()} or the Jelly Bean workaround is used. We
	 * use this in {@link #getTitleHeight()}.
	 */
	 ...
	 * 
	 * @param v
	 *            The view to set or null for removing the title bar view.
	 */
	public void setEmbeddedTitleBarCompat(View v) {
		if (checkedForNativeSetEmbeddedTitleBarMethod
				&& nativeSetEmbeddedTitleBarMethod == null) {
			setEmbeddedTitleBarJellyBean(v);
		} else {
			try {

				if (nativeSetEmbeddedTitleBarMethod == null) {
					nativeSetEmbeddedTitleBarMethod = getClass().getMethod(
							"setEmbeddedTitleBar", View.class);
	...
	protected int getVisibleTitleHeightCompat() {
		if (mTitleBar == null && nativeGetVisibleTitleHeightMethod != null) {
			try {
				return (Integer) nativeGetVisibleTitleHeightMethod.invoke(this);
			} catch (Exception e) {
			}
		}
	 ...
	 * 
	 * <p>
	 * In the Jelly Bean workaround we need this method because we have to
	 * implement the {@link TitleBarDelegate} interface. But by implementing
	 * this method we override the hidden {@code getTitleHeight()} of the
	 * {@link WebView}s in older Android versions. <br>
	 * What we should do, is return the title height on Jelly Bean and call
	 * through to the parent class on older Android versions. But this would
	 * require even more trickery, so we just inline the parent functionality
	 * which simply calls {@link View#getHeight()}. This is exactly what we do
	 * on Jelly Bean anyway.
	 * </p>
	 */
	@Override
