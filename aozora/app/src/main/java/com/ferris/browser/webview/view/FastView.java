...
package com.ferris.browser.webview.view;
...
import com.ferris.browser.constant.StartPage;
import com.ferris.browser.controller.BrowserController;
import com.ferris.browser.download.LightningDownloadListener;
import com.ferris.browser.preference.PreferenceManager;
import com.ferris.browser.utils.AdBlock;
import com.ferris.browser.utils.IntentUtils;
import com.ferris.browser.utils.Utils;
import com.ferris.browser.webview.FWebView;
...
public class FastView {
	...
	protected BrowserController mBrowserController;
	protected GestureDetector mGestureDetector;
	protected final Activity mActivity;
	protected WebSettings mSettings;
	protected static String mHomepage;
	protected static String mDefaultUserAgent;
	protected static Bitmap mWebpageBitmap;
	protected static PreferenceManager mPreferences;
	protected final AdBlock mAdBlock;
	protected IntentUtils mIntentUtils;
	protected final Paint mPaint = new Paint();
	protected boolean isForegroundTab;
	protected boolean mTextReflow = false;
	protected boolean mInvertPage = false;
	protected static final int API = android.os.Build.VERSION.SDK_INT;
//	private static final int SCROLL_UP_THRESHOLD = Utils.convertDpToPixels(10);
//	private static final int SCROLL_DOWN_THRESHOLD = Utils
//			.convertDpToPixels(100);
	protected static final float[] mNegativeColorArray = { -1.0f, 0, 0, 0, 255, // red
	...
	@SuppressLint("NewApi")
	public FastView(Activity activity, String url, boolean darkTheme) {

		mActivity = activity;
		mWebView = new FWebView(activity);
		mTitle = new Title(activity, darkTheme);
		mAdBlock = AdBlock.getInstance(activity.getApplicationContext());

		mWebpageBitmap = Utils.getWebpageBitmap(activity.getResources(),
				darkTheme);
		...
		if (url != null) {
			...
			showHomeOrWebView(true);
		}
	}

	/**
	 * 是网页还是安卓页面
	 * @param isHomeOrWebView
	 */
	public void showHomeOrWebView(Boolean isHomeOrWebView){
	 ...
	 * @param bottom_bar
	 */
	public void setHeadorFooderView(View mTitleBar,View bottom_bar){
		mWebView.setEmbeddedTitleBarCompat(mActivity.getLayoutInflater().inflate(
				R.layout.header, null));
		mWebView.setTitleBar(mTitleBar);
//		mWebView.setBottomBar(bottom_bar);
	}
	...
	public synchronized void initializePreferences(Context context) {
		...
		case 3:
			...
			break;
		case 4:
			mSettings.setUserAgentString(mPreferences
					.getUserAgentString(mDefaultUserAgent));
			break;
		}
		...
		if (mPreferences.getTextReflowEnabled()) {
			...
			if (API >= android.os.Build.VERSION_CODES.KITKAT) {
				try {
					mSettings
							.setLayoutAlgorithm(LayoutAlgorithm.TEXT_AUTOSIZING);
				} catch (Exception e) {
					// This shouldn't be necessary, but there are a number
					// of KitKat devices that crash trying to set this
					Log.e(Constants.TAG,
							"Problem setting LayoutAlgorithm to TEXT_AUTOSIZING");
				}
			}
		} else {
			mTextReflow = false;
		...
		mSettings
				.setLoadWithOverviewMode(mPreferences.getOverviewModeEnabled());
...
//		}
	...
	private void cacheFavicon(Bitmap icon) {
		...
		Log.d(Constants.TAG,
				"Caching icon for " + Utils.getDomainName(getUrl()));
...
//		}
		...
		public void onPageStarted(WebView view, String url, Bitmap favicon) {
			...
			mWebView.disableScrollTitle();//页面开始加载的时候禁止滚动网页
					...
					mIsRunning = view.postDelayed(new Runnable() {
						...
						public void run() {
							...
							view.evaluateJavascript(
									Constants.JAVASCRIPT_TEXT_REFLOW, null);
							mIsRunning = false;
						}
		...
		public void onFormResubmission(WebView view,
				@NonNull final Message dontResend, final Message resend) {
			AlertDialog.Builder builder = new AlertDialog.Builder(mActivity);
...
//		}
	...
	public class Title {
		...
		public Title(Context context, boolean darkTheme) {
			mDefaultIcon = Utils.getWebpageBitmap(context.getResources(),
					darkTheme);
